...
const Lose = () => {
  ...
  return (
    <div className='flex h-full w-full flex-col justify-center gap-4'>
      <PageHeader text="WHERE'S ADAM" bg='bg-primary-blue' />
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className='self-center whitespace-nowrap text-center text-sm font-extrabold tracking-tight text-primary-green'>
          {isChallenger
            ? `You put Adam ${getAnswer(challenger_answer)}`
            : `${shortenAddress(challenger_address)} put Adam ${getAnswer(
                challenger_answer
              )}`}
        </div>
        <div className='self-center whitespace-nowrap text-center text-sm font-extrabold tracking-tight text-primary-green'>
          {!isChallenger
            ? `You guessed Adam was ${getAnswer(opponent_answer)}`
            : `${shortenAddress(opponent_address)} guessed Adam was ${getAnswer(
                opponent_answer
              )}`}
        </div>
      </div>
